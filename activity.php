...
$data = [];
$result = pg_query_params($db, "SELECT started_at, ended_at, uplink_traffic + downlink_traffic AS traffic, 1 AS service, protocol, client_address FROM ports.traffic INNER JOIN network.zones_servers USING (server_id) INNER JOIN ports.ports USING (port, zone_id) WHERE user_id = $1 AND started_at > now() - '1 month' :: INTERVAL ORDER BY client_address, started_at", [$user_id]) or die("query failed" . pg_last_error($db));
...
$result = pg_query_params($db, "SELECT acctstarttime AS started_at, acctstoptime AS ended_at, acctsessiontime AS duration, acctinputoctets + acctoutputoctets AS traffic, 3 AS service, split_part(HOST (framedipaddress :: INET), '.', 3 ) :: INT >> 4 AS protocol, split_part(callingstationid, '=', 1) :: INET AS client_address FROM radius.radacct RIGHT JOIN radius.radcheck USING (username) WHERE id = $1 AND acctstarttime > now() - '1 month' :: INTERVAL", [$user_id]) or die("query failed" . pg_last_error($db));
while ($row = pg_fetch_assoc($result)) {
